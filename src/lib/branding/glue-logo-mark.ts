const GLUE_LOGO_PATHS = [
  ...
  "M75 302.5C75 303.8 86.7 304 174 304C261.3 304 273 303.8 273 302.5C273 301.2 261.3 301 174 301C86.7 301 75 301.2 75 302.5Z",
] as const;

export const buildGlueLogoSvg = (fillColor: string): string => {
  const paths = GLUE_LOGO_PATHS.map(
    (d) => `<path d="${d}" fill="${fillColor}" />`
  ).join("");

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 344 347" fill="none">${paths}</svg>`;
};
...
const loadImage = (src: string): Promise<HTMLImageElement> =>
  new Promise((resolve, reject) => {
    ...
    img.onerror = () =>
      reject(new Error("glue-logo-mark: failed to rasterize logo"));
    img.src = src;
  });

/** Rasterizes the GLUE wordmark for PDF/image embedding. */
export const loadGlueLogoDataUrl = async (
  fillColor: string,
  widthPx = 344
): Promise<string> => {
  ...
  if (!ctx) {
    throw new Error("glue-logo-mark: no 2d context");
  }
